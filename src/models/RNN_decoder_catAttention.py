...
class DecoderRNN(nn.Module):
    ...
    def __init__(self, num_features, embedding_dim, category_dim, hidden_dim, vocab_size, p =0.5, cat_attention=False):
        ...
        # add attention layer
        if cat_attention:
            self.attention = TranslateAttention(num_features + category_dim, hidden_dim)
            self.init_h = nn.Linear(num_features + category_dim, hidden_dim)
            self.init_c = nn.Linear(num_features + category_dim, hidden_dim)
        else:
